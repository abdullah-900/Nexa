...
const handlecodSelected=(event:React.ChangeEvent<HTMLInputElement>)=>{
  setCodSelected(event.target.checked)
}

const handleCreditCardSelected=(event:React.ChangeEvent<HTMLInputElement>)=> {
setCreditCardSelected(event.target.checked)
}
  return (
    ...
    <section className="w-[90%] md:[37.5rem]  lg:w-[52.5rem] rounded-xl bg-f6 p-5 ml-5 md:ml-9 lg:ml-[6.25rem] pl-[1.75rem] py-[2.8125rem] pr-[3rem]">
      <article className="w-full lg:w-[47.75rem]">
        <div className={`${creditCardSelected?"opacity-30":""}  flex items-center gap-[1.1875rem]`}>
          <input disabled={creditCardSelected?true:false} className="rounded-full cursor-pointer" type="checkbox" id="Cash on delivery" checked={codSelected} onChange={handlecodSelected} />
        ...
        <div className="flex flex-col gap-[1.875rem] w-full">
          <div className={`${codSelected?'opacity-30':''}  flex items-center gap-[1.1875rem]`}>
            <input  disabled={codSelected?true:false} className= "rounded-full  cursor-pointer" type="checkbox" id="Credit Card" checked={creditCardSelected} onChange={handleCreditCardSelected}  />
    ...
    <button className="md:ml-[6.25rem]  mt-[3.125rem] mb-[6.25rem] py-4 px-5 bg-8a rounded-lg text-[1.125rem] font-medium font-Causten text-[#FFFFFF]">Place Order</button>
    </div>
</>
